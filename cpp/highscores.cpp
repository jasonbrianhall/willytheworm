...
HighScoreManager::~HighScoreManager() { save_scores(); }
...
void HighScoreManager::load_scores() {
  ...
  try {
    ...
    std::vector<HighScore> previous_scores =
        permanent_scores; // Keep old scores
    ...
    for (const auto &old_score : previous_scores) {
      if (std::none_of(permanent_scores.begin(), permanent_scores.end(),
                       [&](const HighScore &s) {
                         return s.name == old_score.name &&
                                s.score == old_score.score;
                       })) {
...
void HighScoreManager::save_scores() {
  ...
  try {
    ...
    for (size_t i = 0; i < permanent_scores.size(); i++) {
      file << "    [\"" << permanent_scores[i].name << "\", "
           << permanent_scores[i].score << "]";
      if (i < permanent_scores.size() - 1)
        file << ",";
      file << "\n";
    }
    file << "  ],\n";
    ...
    for (size_t i = 0; i < daily_scores.size(); i++) {
      file << "    [\"" << daily_scores[i].name << "\", "
           << daily_scores[i].score << "]";
      if (i < daily_scores.size() - 1)
        file << ",";
      file << "\n";
    }
    file << "  ]\n";
...
void HighScoreManager::parse_scores_json(const std::string &json_content) {
  ...
  std::sort(
      permanent_scores.begin(), permanent_scores.end(),
      [](const HighScore &a, const HighScore &b) { return a.score > b.score; });
  std::sort(
      daily_scores.begin(), daily_scores.end(),
      [](const HighScore &a, const HighScore &b) { return a.score > b.score; });
}
...
std::string HighScoreManager::get_score_description(int score) {
  if (score < 1000) {
    ...
    return "Absolutely fantastic! You should consider a career as an "
           "earthworm!";
  }
}
...
void WillyGame::draw_high_score_entry_screen(
  ...
  std::string score_text =
      "Your score for this game is " + std::to_string(score) + "...";
...
void WillyGame::draw_high_score_display_screen(
    const Cairo::RefPtr<Cairo::Context> &cr) {
  // Get menubar height to use as offset
  Gtk::Requisition menubar_min, menubar_nat;
  menubar.get_preferred_size(menubar_min, menubar_nat);
  ...
  int table_x =
      (GAME_SCREEN_WIDTH * GAME_CHAR_WIDTH * scale_factor - table_width) / 2;
  ...
  for (int i = 0; i < 10 && i < (int)permanent_scores.size(); i++) {
    ...
    oss << std::setw(2) << (i + 1) << "     " << std::setw(5)
        << permanent_scores[i].score << "     " << permanent_scores[i].name;
  ...
  for (int i = 0; i < 10 && i < (int)daily_scores.size(); i++) {
    std::ostringstream oss;
    oss << std::setw(2) << (i + 1) << "     " << std::setw(5)
        << daily_scores[i].score << "     " << daily_scores[i].name;
