...
public:
  SpriteIterator() : current_index(0) {
    ...
    sprite_names = {"WILLY_RIGHT", "WILLY_LEFT", "PRESENT",    "LADDER",
                    "TACK",        "UPSPRING",   "SIDESPRING", "BALL",
                    "BELL",        "BALLPIT",    "EMPTY"};
  ...
  std::string current() const { return sprite_names[current_index]; }

  void next() { current_index = (current_index + 1) % sprite_names.size(); }
  ...
  size_t size() const { return sprite_names.size(); }
};
...
private:
  ...
  // Editor states
  enum class EditorState { INTRO, EDITING } editor_state;
};

WillyEditor::WillyEditor()
    : vbox(Gtk::ORIENTATION_VERTICAL), editor_state(EditorState::INTRO),
      scale_factor(editor_options.scale_factor), current_level_num(1),
  ...
  signal_size_allocate().connect(
      sigc::hide(sigc::mem_fun(*this, &WillyEditor::on_window_resize)));
...
bool WillyEditor::on_scroll_event(GdkEventScroll *event) {
  ...
  if (event->direction == GDK_SCROLL_UP) {
    ...
    std::cout << "Scroll up - previous sprite: " << sprite_iterator.current()
              << std::endl;
  } else if (event->direction == GDK_SCROLL_DOWN) {
    sprite_iterator.next();
    std::cout << "Scroll down - next sprite: " << sprite_iterator.current()
              << std::endl;
  }
...
bool WillyEditor::on_button_press(GdkEventButton *event) {
  ...
  if (event->button == 1) { // Left click - place sprite
    ...
    if (current_sprite == "BALLPIT") {
      ...
      std::cout << "Set primary ball pit at (" << row << ", " << col << ")"
                << std::endl;
    }
...
std::pair<int, int> WillyEditor::screen_to_grid(double screen_x,
                                                double screen_y) {
...
void WillyEditor::place_sprite(int row, int col,
...
void WillyEditor::load_level(int level_num) {
  ...
  // Set up the preview sprite in top-right corner (column 40, outside the game
  // area)
  place_sprite(0, 40, sprite_iterator.current());
...
void WillyEditor::ensure_level_exists() {
  ...
  if (!level_loader->level_exists(current_level)) {
    ...
    // Create level by setting at least one tile - we'll set all to EMPTY
    // initially
...
void WillyEditor::save_level() {
  ...
  if (level_loader->save_levels(editor_options.levels_file)) {
    std::cout << "Level saved successfully to " << editor_options.levels_file
              << std::endl;
  } else {
...
void WillyEditor::quit_editor() { hide(); }
...
WillyEditorApplication::WillyEditorApplication()
    : Gtk::Application("org.example.willyeditor") {}
...
void print_editor_help(const char *program_name) {
  ...
  std::cout << "Notes:\n";
  std::cout << "  - Only one Willy can exist per level (placing a new one "
               "removes others)\n";
  std::cout
      << "  - The last BALLPIT placed becomes the primary ball spawn point\n";
...
bool parse_editor_command_line(int argc, char *argv[]) {
  ...
  while ((c = getopt_long(argc, argv, "hL:S:", long_options, &option_index)) !=
         -1) {
    switch (c) {
    case 'h':
...
void WillyEditor::test_level() {
  ...
  std::cout << "Launching game to test level " << current_level_num
            << " (1 life)" << std::endl;
